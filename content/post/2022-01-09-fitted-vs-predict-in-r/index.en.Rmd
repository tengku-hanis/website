...
However, in `predict(type = "terms")` the values is [centered](https://www.statology.org/center-data-in-r/), thus we have a different values for constant/intercept and for $\beta_1(Sepal.Length)$,$\beta_1(Sepal.Length)$ and so on. For example, the values for intercept for both models are:
