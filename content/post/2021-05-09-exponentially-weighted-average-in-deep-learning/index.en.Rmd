...
I have been reading about lost function and optimisers in Deep Learning for the last couple of days when I stumble upon the term Exponentially weighted average (EWA). So, in this post I aims to explain my understanding of EWA. EWA basically  is an important concept in deep learning and have been used in several optimisers to smoothen the noise of the data.

Let's see the formula for EWA:

```{r, echo=FALSE, out.width='60%', fig.align='center', fig.cap="Formula of exponentially weighted average"}
...
So, some of the optimisers that adopt the approach of EWA are:
...
```{r, echo=FALSE, out.width='60%', fig.align='center', fig.cap="Red box indicate the EWA part"}
knitr::include_graphics('SGD-momentum2.png')
```

2) Adaptive delta (Adadelta) and Root Mean Square Propagation (RMSprop)

Adadelta and RMSprop is proposed in attempt to solve the issue of diminishing learning rate of adaptive gradient (Adagrad) optimiser. The use of EWA in both optimisers actually helps to achieve this. Both optimisers have quite a similar formula, but attached below is the formula for Adadelta.
...
Now, let's go back to EWA. Here, the example of calculation of EWA:
...
So, if we were to varies the value of *B* across the equation, we can do so in R.
...
1) Thee EWA function acts in a decaying manner. 
